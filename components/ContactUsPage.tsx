...
const ContactUsPage = () => {
  ...
  return (
    <div className="min-h-screen w-full rounded-md bg-transparent relative flex flex-col items-center justify-center antialiased py-16 sm:py-20 md:py-24 lg:py-28 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto w-full z-10">
        <TextGenerateEffect
          words="Eager to Connect? Contact Me"
          className="text-center text-2xl sm:text-3xl md:text-4xl lg:text-5xl mb-6 sm:mb-8 md:mb-10"
        />
        <div className="max-w-3xl w-full mx-auto rounded-lg md:rounded-2xl p-4 sm:p-6 md:p-8 shadow-input bg-white dark:bg-black-200">
          <form className="my-8" action={formActions}>
            <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 mb-4">
              <div className="w-full md:w-1/2">
                ...
                />
                {error.firstname && (
                  <span className="mt-2 text-red-500 text-sm">{error.firstname}</span>
                )}
              </div>
              <div className="w-full md:w-1/2">
                ...
                />
                {error.lastname && (
                  <span className="mt-2 text-red-500 text-sm">{error.lastname}</span>
                )}
              </div>
            </div>
            <div className="mb-4">
              ...
              />
              {error.email && (
                <span className="mt-2 text-red-500 text-sm">{error.email}</span>
              )}
            </div>
            <div className="mb-4">
              ...
              />
              {error.subject && (
                <span className="mt-2 text-red-500 text-sm">{error.subject}</span>
              )}
            </div>
            <div className="mb-6 sm:mb-8">
              ...
              />
              {error.query && (
                <span className="mt-2 text-red-500 text-sm">{error.query}</span>
              )}
            </div>
            <div
              className={cn({
